...
#[derive(Eq, PartialEq)]
struct State(usize, i32); // index, steps
impl Ord for State {
    // return smaller steps, then choose by bigger index
...
impl Solution {
    pub fn min_jumps(arr: Vec<i32>) -> i32 {
        ...
        let len = arr.len();
        let mut num_to_indices: HashMap<i32, Vec<usize>> =
            arr.iter()
                .enumerate()
                .fold(HashMap::new(), |mut map, (i, &num)| {
                    map.entry(num).or_insert_with(Vec::new).push(i);
                    map
                });
        for value in num_to_indices.values_mut() {
            value.sort_unstable(); // want to make sure we're pulling the biggest index out of the list for j
                                   // not sure if this actually helps...
        }

        let mut shortest_path: Vec<i32> = (0..len).into_iter().map(|u| u as i32).collect();
        shortest_path[0] = 0;
        pq.push(State(0, 0));

        // 1) check for end before we push into queue, otherwise end will be checked last, since its
        // steps may be longer
        // 2) check shortest path to see if we've visited this location before.  ensure our path is
        //    shorter before pushing into queue
        // 3) ??
        while let Some(State(index, steps)) = pq.pop() {
            if steps + 1 > shortest_path[len - 1] {
                break; // we're always grabbing the state with the least steps
                       // if the least steps is more than we've already found, then all states in
                       // the queue are failures
            }
            // reached the end by going right.
            if index + 1 == len - 1 {
                if shortest_path[len - 1] > steps + 1 {
                    shortest_path[len - 1] = steps + 1;
                    println!("shortest path found: {}", shortest_path[len - 1]);
                }
                continue; //no reason to continue this route
            }

            // right.  dont need to check oob, since oob would mean we're continuing past last
            // index, which we never do
            if shortest_path[index + 1] > steps + 1 {
                pq.push(State(index + 1, steps + 1));
            }

            // left
            if index > 0 && shortest_path[index - 1] > steps + 1 {
                pq.push(State(index - 1, steps + 1));
            }

            // j's
            if let Some(indices) = num_to_indices.get(&arr[index]) {
                'j_loop: for j in indices {
                    // check prev visits
                    if shortest_path[*j] > steps + 1 {
                        // check if j is end
                        if *j == len - 1 {
                            if shortest_path[len - 1] > steps + 1 {
                                shortest_path[len - 1] = steps + 1;
...
mod tests {
    ...
    fn first() {
        ...
        let ans = Solution::min_jumps(input);
        let expected = 1;
